...
# 读数据
route <- fread("D:/data/ISPRS/BUS_ROUTE_DIC.csv")

gps <- fread("D:/data/ISPRS/GPS_DATA.csv")
...
afc <- fread("D:/data/ISPRS/AFC_DATA.csv")
sum(duplicated(afc[, .(card_id, day, time)]))
afc <- unique(afc, by=c("card_id", "day", "time"), fromFirst=TRUE)
# 20036579 records - 8848

# play start
length(unique(afc$card_id))
afcSummary <- afc[, .N, by=.(card_id, day)]
prop.table(table(afcSummary$N))

setkey(afc, card_id, day, time)
afc[, c("time_next", "bus_id_next") := list(c(lead(time, 1L), lead(bus_id, 1L))), by=.(card_id, day)]
dim(afc[bus_id == bus_id_next & time_next - time < 600,])

setkey(gps, bus_id, day, time)
gps[, time_next := lead(time, 1L), by=.(bus_id, day)]
summary(gps$time_next - gps$time)

ggplot(afc[bus_id == 1 & day == 1, ], aes(x = 1:341, y = time)) + geom_point()
ggplot(afc[bus_id == 1 & day == 1, ], aes(x = 1:341, y = timeDiff)) + geom_line()
ggplot(afc[bus_id == 1 & day == 1 & timeDiff < 1800, ], aes(x = 1:334, y = timeDiff)) + geom_line()
# play end
